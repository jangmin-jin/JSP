...
<%
	...
	try{
		...
		if(rs.next()){
			...
			hit = Integer.parseInt(rs.getString("hit"));
		...
		// 두번째 쿼리 : 조회수를 증가사항을 데이터베이스에 적용
		sql = "update board1 set hit=? where seq=?";
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, Integer.toString(hit));
	...
	<div class="contents_sub">
		...
		<div class="btn_area">
			...
			<div class="align_right">
				...
				<input type="button" value="답글쓰기" class="btn_write btn_txt01" style="cursor: pointer;" onclick="location.href='board_reply1.jsp?seq=<%=seq %>&cpage=<%=cpage %>'" />
			</div>
		</div>	
		<!--//게시판-->
	</div>
